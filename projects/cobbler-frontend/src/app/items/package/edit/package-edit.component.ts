...
export class PackageEditComponent implements OnInit {
  ...
  refreshData(): void {
    this.cobblerApiService.get_package(this.name, false, false, this.userService.token).subscribe(value => {
      this.package = value
      ...
      this.packageFormGroup.controls.mtime.setValue(new Date(this.package.mtime * 1000).toString())
      this.packageFormGroup.controls.ctime.setValue(new Date(this.package.ctime * 1000).toString())
  ...
  removePackage(): void {
    this.cobblerApiService.remove_package(this.name, this.userService.token, false).subscribe(value => {
      if (value) {
  ...
  copyPackage(): void {
    this.cobblerApiService.copy_package("", "", this.userService.token)
      .subscribe(value => {
        // TODO
      }, error => {
        // HTML encode the error message since it originates from XML
