...
export class FileEditComponent implements OnInit {
  ...
  refreshData(): void {
    this.cobblerApiService.get_file(this.name, false, false, this.userService.token).subscribe(value => {
      this.file = value
      this.fileFormGroup.controls.name.setValue(this.file.name)
      this.fileFormGroup.controls.uid.setValue(this.file.uid)
      this.fileFormGroup.controls.mtime.setValue(new Date(this.file.mtime * 1000).toString())
      this.fileFormGroup.controls.ctime.setValue(new Date(this.file.ctime * 1000).toString())
  ...
  removeFile(): void {
    this.cobblerApiService.remove_file(this.name, this.userService.token, false).subscribe(value => {
      if (value) {
  ...
  copyFile(): void {
    this.cobblerApiService.copy_file("", "", this.userService.token)
      .subscribe(value => {
        // TODO
      }, error => {
        // HTML encode the error message since it originates from XML
