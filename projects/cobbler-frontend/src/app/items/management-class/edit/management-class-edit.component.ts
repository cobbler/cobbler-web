...
export class ManagementClassEditComponent implements OnInit {
  ...
  refreshData(): void {
    this.cobblerApiService.get_mgmtclass(this.name, false, false, this.userService.token).subscribe(value => {
      this.managementClass = value
      this.managementClassFormGroup.controls.name.setValue(this.managementClass.name)
      this.managementClassFormGroup.controls.uid.setValue(this.managementClass.uid)
      this.managementClassFormGroup.controls.mtime.setValue(new Date(this.managementClass.mtime * 1000).toString())
      ...
      if (typeof this.managementClass.owners === "string") {
        this.managementClassFormGroup.controls.owners_inherited.setValue(true)
      } else {
        this.managementClassFormGroup.controls.owners_inherited.setValue(false)
        this.managementClassFormGroup.controls.owners.setValue(this.managementClass.owners)
      }
      this.managementClassFormGroup.controls.params.setValue(this.managementClass.params)
      this.managementClassFormGroup.controls.files.setValue(this.managementClass.files)
      this.managementClassFormGroup.controls.packages.setValue(this.managementClass.packages)
    }, error => {
  ...
  removeManagementClass(): void {
    this.cobblerApiService.remove_mgmtclass(this.name, this.userService.token, false).subscribe(value => {
      if (value) {
  ...
  copyProfile(): void {
    this.cobblerApiService.copy_mgmtclass("", "", this.userService.token)
      .subscribe(value => {
        // TODO
      }, error => {
        // HTML encode the error message since it originates from XML
