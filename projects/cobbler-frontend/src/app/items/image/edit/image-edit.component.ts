...
export class ImageEditComponent implements OnInit{
  ...
  refreshData(): void {
    this.cobblerApiService.get_image(this.name, false, false, this.userService.token).subscribe(value => {
      this.image = value
      this.imageFormGroup.controls.name.setValue(this.image.name)
      this.imageFormGroup.controls.uid.setValue(this.image.uid)
      this.imageFormGroup.controls.mtime.setValue(new Date(this.image.mtime * 1000).toString())
      this.imageFormGroup.controls.ctime.setValue(new Date(this.image.ctime * 1000).toString())
      this.imageFormGroup.controls.depth.setValue(this.image.depth)
      this.imageFormGroup.controls.network_count.setValue(this.image.network_count)
      this.imageFormGroup.controls.is_subobject.setValue(this.image.is_subobject)
      ...
      this.imageFormGroup.controls.autoinstall.setValue(this.image.autoinstall)
      this.imageFormGroup.controls.breed.setValue(this.image.breed)
      this.imageFormGroup.controls.file.setValue(this.image.file)
      this.imageFormGroup.controls.image_type.setValue(this.image.image_type)
      this.imageFormGroup.controls.os_version.setValue(this.image.os_version)
      if (typeof this.image.boot_loaders === "string") {
        ...
        this.imageFormGroup.controls.boot_loaders.setValue(this.image.boot_loaders)
      }
  ...
  removeImage(): void {
    this.cobblerApiService.remove_image(this.name, this.userService.token, false).subscribe(value => {
      if (value) {
  ...
  copyImage(): void {
    this.cobblerApiService.copy_image("", "", this.userService.token)
      .subscribe(value => {
        // TODO
      }, error => {
        // HTML encode the error message since it originates from XML
